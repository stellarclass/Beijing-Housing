...
```
We also find that, through inspecting some of the original URL, several data seems to be misplaced into a wrong columns such that off-the-chart values are present in these columns. For example, some rows have 2006 bathrooms. It seems more appropriate to say that the construction time for this row is 2006. Moreoever, all these rows have missing values. We infer that these bad data points probabably result from bugs in the crawler. There are only a handfule of such data points. So we drop them directly.
```{r}
...
For the predictive modeling of this dataset, we will attempt to build a model for "price", which is the total price over square meters of the property. We think this would be a good indicator for the general affordability in Beijing city as opposed to "totalPrice". We have divided the dataset into train and test set by the year: all data before 2017 will be the training set and all after 2017 will be test set. This mimics the situation where people use historical data to predict future housing price in Beijing. After all, there is no point in predicting a past transactino because we can simply look it up. Credit: [this report] (https://www.kaggle.com/jonathanbouchet/forecasting-beijing-s-housing).

We also prepared two types of data for traning and testing. First is the data within which the factors are not explicitly dummy-coded. This is for our own convience when writing formula for the glm models.
```{r}
data_train_nodummy <- data.frame(data %>% filter(year<2017))
data_test_nodummy <- data.frame(data %>% filter(year>=2017))
```

The second type of data we need to prepare is the one that explicitly dummy-encodes the factor variables. We need to use this since we will be using advanced modeling package xgboost, which can only handel numeric data. And finally we have defined 5-folds cross-validation as the evaluation methods for our modeling tasks.
```{r}
...
model_glm$results
```
We see that the linear regression models are not particular good. In terms of Rsuqared, it reaches 0.77. The glmnet model, which comes with regularization, is no more better than plain glm model. And we won't brother to show the glmnet model results here. This suggests that we have a underfitting problem resulting from a model that is too simple. We also noticed that we have have only experimented a 1st order linear model. But before we attempt to make a higher-order model, there is another problem, that is, which predictors do we choose to combine and make high order predictors? Let's make some guess for the formula:
...
As it turns out, the 5th-order linear model become worse. If we go down this path, one big hyper-parameter we have to tune seems to be number of order and the combination of predictors that make high order predictors. And it seems the only way to do this is exhasitive search. We will look for two more complex models to see if there is smarter algorithms that can give us better results.

### Decision Tree Models
Previously we mentioned that a linear regression has underfitting model. We now attempt to fit the dataset using a decision tree model, with the "cp" as the hyperpameter tunning.
```{r}
...
Next we investigate a powerful but complext models, xgboost. As we will see shortly, out-of-box xgboost on this dataset yield ~15% reduction in RMSE as compared to the decision tree model. With some tuning, we are able to yield 18% reduction in RMSE as compared to the rpart model. We have follows [this](https://www.kaggle.com/pelkoja/visual-xgboost-tuning-with-caret) tutorial in learning how to tune the xgboost model. 
...
```
We have obtained the best hyperparamters for xgboost according to 5-folds cross validation. Final comparison of RMSE of three models based on cross-validation.
```{r}
...
## VI. Evaluation
We now apply the models we have previously trained on a "future dataset" - the transcation data from 2017 and beyond. Recall that we have trained the models using data before 2017 and try to predict housing price after 2017. In the following, the true price vs predicted price are plotted. The blue line is the ideal prediction, where all predicted price match 100% with the true price.
```{r}
model = xgb_best
...
df_test_results
```
It is interesting to see from these plots that:
