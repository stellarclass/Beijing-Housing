...
### Loading data
data=read.csv(paste(data_dir,"imputed_data.csv",sep="/"), header = TRUE, sep = ",",na.strings = c("NA", "nan","#NAME?"))
```
...
It does not make sense to have more than one ladder per unit, so anything greater than a 1:1 ratio is considered an outlier.
...
data$floor = gsub("¨¦????", "low", data$floor)
data$floor = gsub("????a????£¤", "unknown", data$floor)
